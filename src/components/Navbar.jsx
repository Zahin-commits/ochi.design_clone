...
const Navbar = ({isDark,setIsDark}) => {
    ...
    const menu = ["Services", "Our Wrok", "About Us", "Insights", "Contact"]

  return (
    <div ref={navRef} className={`fixed z-[99] w-full px-20 py-8 flex justify-between items-center backdrop-blur-sm bg-[#18181ba6] 
    `}>
        <div className='logo'> 
        ...
        <div className="links flex gap-10">
            {menu.map((item,index)=>( 
                <a key={index} className={`text-lg relative h-[25px] overflow-hidden nav-hover-btn font-light ${index === 4 && 'ml-32'} 
                  ${(index===1) || (index===2) ? 'w-[92px]':'w-[77px]'}`}>
                   <div className='w-[100px] absolute top-0 left-1/2 -translate-x-1/2 transition-all duration-[350ms] ease-[cubic-bezier(0.87, 0, 0.13, 1)] hover:top-[-110%]'>
